...
const ensureEdgeFunctions = async () => {
  const healthUrl = `http://127.0.0.1:${supabasePort}/functions/v1/ledger-health`
...
const ensureVite = async () => {
  ...
  startManagedProcess('Friendly Ledger dev server', 'npm', ['run', 'dev', '--', '--host', '0.0.0.0'])
  await waitFor('Friendly Ledger dev server', () => httpOk(appUrl))
}
...
const printEndpoints = async (lanAddress) => {
  ...
  await checkEndpoint('Health local', `http://${local}:${supabasePort}/functions/v1/ledger-health`)
  if (lan) {
    await checkEndpoint('Health LAN', `http://${lan}:${supabasePort}/functions/v1/ledger-health`)
  }
